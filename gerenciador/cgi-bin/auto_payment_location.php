...
foreach ($locations->data as $k => $v) {
    ...
    /** Contrato Vigente - Gerar boleto 15 dias antes do vencimento */
    if ($v["active"] == "yes" && $v["payment_method"] == "ticket" && $resto > 0 && $dateNow == $lastTendays) {
        ...
        /** Contrato Vencido -  Cancelar e disparar email */
        $info["post"]["active"] = "no";
        $info["post"]["modified_at"] = date("Y-m-d H:i:s");
        $info["post"]["modified_by"] = "1";

        $location->populate($info["post"]);
        $location->save();

        $page = strtr(file_get_contents(constant("cFurniture") . "mail/alert_location_expired.html"), array(
            "#HOST#" => constant("cFurniture") . "mail/alert_location_expired.html",
            "#NOME#" => $v["first_name"] . " " . $v["last_name"],
            "#CONTRACT#" => $v["n_contract"]
        ));
        ...
        $messages_model = new messages_model();
        $messages_model->populate(array(
            "name" => "SISMOB - Aviso de contrato vencido sob o n° " . $v["n_contract"],
            "scheduled_at" => date("Y-m-d H:i:s"),
            "mailboxes" => serialize(array(
                "Address" => array(
                    "name" => $v["first_name"] . " " . $v["last_name"],
