...
class tests_controller{
	...
	public function display( $info ){		
		...
		$form = array(
			...
			, "titlenew" => "Nova Aula"
			, "new" => $GLOBALS["newtest_url"]
			, "search" => $GLOBALS["tests_url"]
			, "action" => set_url( $GLOBALS["test_url"] , $done )
		) ;
		include( constant("cRootServer") . "ui/common/header.inc.php");
		include( constant("cRootServer") . "ui/common/head.inc.php");
		include( constant("cRootServer") . "ui/page/tests.php");
	...
	public function form( $info ){	
		...
		if( isset( $info["idx"] ) && (int)$info["idx"] > 0 ){
			...
			$boiler->load_data();
			$boiler->attach(array("questions"));
			$boiler->set_paginate( array(1) ) ;
			$data = current( $boiler->data ) ;
			$form["title"] = "Editar Avaliação";
			$form["url"] = sprintf( $GLOBALS["test_url"] , $info["idx"] ) ;
		}
		include( constant("cRootServer") . "ui/common/header.inc.php");
		include( constant("cRootServer") . "ui/common/head.inc.php");
		include( constant("cRootServer") . "ui/page/test.php");
		include( constant("cRootServer") . "ui/common/footer.inc.php");
		include( constant("cRootServer") . "ui/common/list_actions.php");
		include( constant("cRootServer") . "ui/common/foot.inc.php");
	}
	...
	public static function display_in_section($info){			
		...
		$section->load_data();
		$section->attach(array("tests"),true);
		$section_tests = $section->data;
