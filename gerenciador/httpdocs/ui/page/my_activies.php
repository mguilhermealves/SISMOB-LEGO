    ...
    <div class="col-sm-12">
        <div class="col-sm-12">
            <blockquote class="blockquote">
                <p class="mb-0">Minhas atividades</p>
            </blockquote>
            <hr>
        </div>
                    ...
                    <div class="col-sm-12">
                        <table class="table table-striped table-inverse">
                            ...
                            <tfoot>
                                <tr>
                                    <th colspan="5">
                                        <form  id="frm_filter" method="GET" action="<?php print( $GLOBALS["myactivities_url"] ) ?>" style="display: none">
                            ...
                            <tbody>                                
                                <?php foreach($data as $activie): ?>
                                    <tr>
                                        ...
                                        <td><?php echo isset($activie["credits"]) ? is_empty_or_null($activie["credits"]) ? "-" : $activie["credits"] : "-" ?></td>
                                        <td><?php echo isset($activie["post"]["ActivityDetails"]["startDate"]) ? is_empty_or_null($activie["post"]["ActivityDetails"]["startDate"] ) ? "-" : date("d/m/Y", strtotime($activie["post"]["ActivityDetails"]["startDate"])) : '-' ?></td>
                                        <?php 
                                            if(isset($activie["status"])){
                                                switch($activie["status"]){
                                                    case 'Em Analise':
                                                        echo '<td class="text-secondary"><i class="fa fa-clock"></i> Em Analise</td>';
                                                        break;
                                                    case 'Revisar':
                                                        printf( '<td class="text-warning"><a href="' . $GLOBALS["registrodeatividade_url"] . '"><i class="fa fa-exclamation-circle"></i> Revisar</a></td>' , $activie['idx'] ) ;
                                                        break;
                                                    case 'Aprovado':
